...
```{r,include=FALSE}
alldat$delta_chloro[alldat$delta_chloro < -5] <- NA

```

#Summary
```{r,include=FALSE}
ggplot(data = alldat, aes(x = year, y = delta_chloro, color = lake)) + geom_line() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  labs(title = "Change in Chlorophyta over time (data values)",
       x = "year",
...
# Model change in Chlorophyta based on lake, year and species
```{r,include=FALSE}
ggplot(data = alldat, aes(x = delta_chloro)) + geom_histogram(binwidth = 1)
summary(alldat$delta_chloro)

mod1 <- lm(delta_chloro ~ lake + year_cent + buffalo_kg_ha * carp_kg_ha, data = alldat)
resid_panel(mod1)
Anova(mod1, type = 3)
summary(mod1)
```
...
#Look at year_cent instead of lake
##emm_lake <- emmeans(mod1, specs = "year_cent")
