...
#Summary
```{r,include=FALSE}
ggplot(data = alldat, aes(x = year, y = delta_tp, color = lake)) + geom_line() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  labs(title = "Change in TP over time (data values)",
       x = "year",
       y = "change in TP")
...
#Model change in TP based on lake, year and species
```{r,include=FALSE}
ggplot(data = alldat, aes(x = delta_tp)) + geom_histogram(binwidth = 1)
summary(alldat$delta_tp)

mod1 <- lm(delta_tp ~ lake + year_cent + buffalo_kg_ha * carp_kg_ha, data = alldat)
resid_panel(mod1)
Anova(mod1, type = 3)
summary(mod1)
```
