...
b <- ggplot(harvest.dat, aes(x=Year,y= Pounds.Acre, fill = Species))+
            geom_bar(stat= "identity", colour = "black")+
...
- Calculate Mean TSI (average of TSI for TP, Secchi, Chla) taken from the June measurements. When a measurement from June is not available, we took the next closest measurement to one full year after the previous year's measurement date. 

- From those mean TSI values, calculate the chanage in TSI from year a to year b. 
...
change.dat.m.a<-melt(yearchange.dat[,c(1,10,13,14)])
...
change.dat.m.a<-melt(yearchange.dat[,c(1,11,12)])

b<-ggplot(change.dat.m.a, aes(x=value))+
      geom_histogram(colour = "black", fill = "white")+
      geom_density(alpha=.2, fill = "#FF6666")+
            facet_grid(Lake~variable, scales = "free")+
            theme(panel.grid.major = element_blank(), 
...
change.dat.m.a<-melt(yearchange.dat[,c(1,8,9,16,17,18,19)])

c<-ggplot(change.dat.m.a, aes(x=value))+
      geom_histogram(colour = "black", fill = "white")+
      geom_density(alpha=.2, fill = "#FF6666")+
            facet_grid(Lake~variable, scales = "free")+
            theme(panel.grid.major = element_blank(), 
