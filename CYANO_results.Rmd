...
#Summary
```{r, include=FALSE}
ggplot(data = alldat, aes(x = year, y = delta_cyano, color = lake)) + geom_line() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  labs(title = "Change in Cyanophyta over time (data values)",
       x = "year",
...
#Model change in Cyanophyta based on lake, year and species
```{r, include=FALSE}
ggplot(data = alldat, aes(x = delta_cyano)) + geom_histogram(binwidth = 1)
summary(alldat$delta_cyano)

mod1 <- lm(delta_cyano ~ lake + year_cent + buffalo_kg_ha * carp_kg_ha, data = alldat)
resid_panel(mod1)
Anova(mod1, type = 3)
summary(mod1)
```
