...
```{r,include=False}
alldat$delta_chla[alldat$delta_chla < -200] <- NA
alldat$delta_chla[alldat$delta_chla > 300] <- NA
```

#Summary
```{r,include=FALSE}
ggplot(data = alldat, aes(x = year, y = delta_chla, color = lake)) + geom_line() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  labs(title = "Change in Chlorophyll a over time (data values)",
       x = "year",
       y = "change in Chl a")
...
```{r,include=FALSE}
ggplot(data = alldat, aes(x = delta_chla)) + geom_histogram(binwidth = 1)
summary(alldat$delta_chla)

mod1 <- lm(delta_chla ~ lake + year_cent + buffalo_kg_ha * carp_kg_ha, data = alldat)
resid_panel(mod1)
Anova(mod1, type = 3)
summary(mod1)
```
