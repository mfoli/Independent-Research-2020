...
#Summary
```{r,include=FALSE}
ggplot(data = alldat, aes(x = year, y = delta_tn, color = lake)) + geom_line() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  labs(title = "Change in TN over time (data values)",
       x = "year",
       y = "change in TN")
...
#Model change in TN based on lake, year and species
```{r,include=FALSE}
ggplot(data = alldat, aes(x = delta_tn)) + geom_histogram(binwidth = 1)
summary(alldat$delta_tn)

mod1 <- lm(delta_tn ~ lake + year_cent + buffalo_kg_ha * carp_kg_ha, data = alldat)
resid_panel(mod1)
Anova(mod1, type = 3)
summary(mod1)
```
