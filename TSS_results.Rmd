...
```{r,include=FALSE}
alldat$delta_tss[alldat$delta_tss > 30] <- NA

```

#Summary
```{r,include=FALSE}
ggplot(data = alldat, aes(x = year, y = delta_tss, color = lake)) + geom_line() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  labs(title = "Change in TSS over time (data values)",
       x = "year",
       y = "change in TSS")
...
#Model change in TSS based on lake, year and species
```{r,include=FALSE}
ggplot(data = alldat, aes(x = delta_tss)) + geom_histogram(binwidth = 1)
summary(alldat$delta_tss)

mod1 <- lm(delta_tss ~ lake + year_cent + buffalo_kg_ha * carp_kg_ha, data = alldat)
resid_panel(mod1)
Anova(mod1, type = 3)
summary(mod1)
```
