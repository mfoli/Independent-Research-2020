...
Watershed Area (acres)    | 296           | 3516             | 129            | 1838                 | 6890
...
harvest.dat$Kilograms <- as.integer(harvest.dat$Kilograms)
...
harvest.dat$Kilograms[harvest.dat$Kilograms == "."] <- 0
harvest.dat$kilograms<-as.numeric(as.character(harvest.dat$Kilograms))
summary(harvest.dat$Kilograms)
...
b <- ggplot(harvest.dat, aes(x=Year,y= Std.Removal.lbs, fill = Species))+
            geom_bar(stat= "identity", colour = "black")+
                labs(title = "Harvest in Kilograms per Acre through Time",
                 x="Date",
                 y="Kilograms per Acre Removed") +
            facet_grid(Lake~.)+
            scale_y_continuous(labels=comma)+
            theme(panel.grid.major = element_blank(), 
                              panel.grid.minor = element_blank(),
